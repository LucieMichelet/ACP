# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 22:20:58 2022
...
def load_data(file_name):
    """
    fonction qui transforme le fichier iris.csv en une matrice numpy E sutilisable pour la suite
    :param file_name: nom du fichier contenant la base de donnée iris
    :return: label, species, E
    """

    if file_name == "iris.csv":
        # data est une matrice / tableau contenant toute les informations du fichier .csv
        data = np.genfromtxt(file_name, dtype=str, delimiter=',')

        # les col_names sont les entetes des colonnes du tableau (1, n)
        col_names = data[0, :-1]

        # label est un vecteur colonne de taille (m, 1) qui donne la categorie de fleur en question : 0, 1 ou 2
        label = data[1:, -1].astype('float')

        # E est la matrice de taille (m, n) des données statistique que nous allons traiter
        # on s'assure que les valeurs sont des reels avec le .astype('float')
        E = data[1:, :-1].astype('float')
        return col_names, label, E

    if file_name == "Howellmod.csv":
        data = np.genfromtxt(file_name, dtype=str, delimiter=',')
        label = data[1:, 0]
        col_names = data[0, 1:]
        E = data[1:, 1:].astype('float')

        # on transforme les label en chiffre 1, 2, 3, 4 ...
    ...
    if file_name == "Presidentielle_2017_1e.xlsx":
        data = pd.read_excel("Presidentielle_2017_1e.xlsx",
                             skiprows=3,
                             skipfooter=30000,
                             usecols=[0, 9, 20, 22, 27, 29, 34, 36, 41, 43, 48, 50, 55, 57, 62, 64, 69, 71, 76, 78, 83, 85, 90, 92])
        m, n = data.shape
        keys = data.keys()

        col_names = ['BLANCS', 'LE PEN', 'MACRON', 'LASSALLE', 'CHEMINADE', 'MÉLENCHON', 'FILLON', 'DUPONT-AIGNAN', 'HAMON', 'ASSELINEAU', 'ARTHAUD', 'POUTOU']
        E = np.zeros((m, len(col_names)), dtype=int)

        for i in tqdm.trange(m):
            for j in range(1, len(keys), 2):
                nom, voix = data.loc[i, [keys[j], keys[j + 1]]]
                idex_nom = col_names.index(nom)
                E[i, idex_nom] = voix

        return col_names, None, E

    if file_name == 'Cotations20211218.csv':
        data = pd.read_excel('Cotations20211218.csv', usecols=[0, 9, 20, 22, 27, 29, 34, 3])
